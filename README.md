...
✅ **已發布 v0.4.5** — 新增 AI 輔助校稿功能（關閉字幕/會議模式時自動透過 LLM 修正同音字、截斷句子、標點等 ASR 錯誤）、支援 Ollama / OpenAI / 智譜等多種後端。
...
- **會議模式** 🧪 — 同時擷取麥克風（近端）與系統音訊（遠端），雙 Overlay 即時顯示雙方辨識結果，內建回音抑制，關閉時自動儲存帶時間戳與來源標示的合併文字記錄
