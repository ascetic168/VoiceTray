# VoiceTray

**[正體中文](README.md)** | **[简体中文](README.zh-CN.md)** | **[English](README.en.md)**

> 語音辨識輸入工具 — 純 CPU 即可流暢運行，無需 GPU。可全本地、無雲端運行（適合企業或保密機關內部使用），低延遲串流辨識 + SenseVoice 文字修正

## 專案狀態

✅ **已發布 v0.4.7** — 會議模式新增聲紋辨識功能，使用 eres2net-v2 模型自動識別不同發言人，在 overlay 與記錄檔中標記每位說話人（Speaker_1, Speaker_2...），SQLite 資料庫持久化，跨會議記憶已知說話人。

## 為什麼使用這個程式

與市面上其他語音輸入工具相比，VoiceTray 解決了三個核心痛點：

### 1. 全本地運行，零隱私疑慮，無需 GPU
所有語音辨識都在本機完成，音訊不會上傳到任何伺服器。採用串流 Zipformer 即時辨識搭配 SenseVoice 離線修正的雙引擎架構，**純 CPU 即可流暢運行，無需獨立顯示卡或 CUDA 環境**，無需網路、無需 API 金鑰，下載模型後即可離線使用。

### 2. 串流 + 離線雙引擎協作，即時回饋零延遲
獨創的串流-離線協作架構：
- **串流引擎**（Zipformer）：按下熱鍵的瞬間即開始即時辨識，邊說邊顯示結果
- **離線引擎**（SenseVoice）：放開熱鍵後，對完整音訊進行高精度辨識，自動替換串流結果
- 串流引擎負責即時反饋，離線引擎負責最終品質，兩者無縫協作

### 3. 單鍵 CTRL，智能判斷使用意圖
只需按住 Ctrl 鍵即開始錄音，程式會自動判斷您的使用意圖：
- 按下 Ctrl 後 2 秒內按其他鍵 → 自動取消，不影響 Ctrl+C、Ctrl+V 等快捷鍵
- 過短的錄音（< 0.8 秒）自動丟棄，避免誤觸產生垃圾文字
- 貼上後自動恢復原有剪貼簿內容，不影響使用者原有的複製貼上工作流

## 功能特色

- **全本地運行** — 串流 Zipformer + SenseVoice 雙引擎，全程離線，零隱私疑慮
- **純 CPU 即可流暢運行** — 無需 GPU、無需 CUDA，一般筆電或桌機都能即時辨識
- **串流即時反饋** — 邊說邊顯示辨識結果，Overlay 浮動視窗即時呈現
- **SenseVoice 品質修正** — 放開熱鍵後自動用高精度離線引擎替換串流結果
- **SenseVoice 內建標點** — 啟用 ITN 功能直接輸出。，？等標點符號，無需額外模型，辨識速度更快
- **語音活動偵測** — Silero VAD 自動過濾靜音停頓，消除辨識垃圾字符（可調整敏感度）
- **離線辨識** — 無需網路及 API 金鑰，首次啟動自動下載模型（含 VAD 模型）
- **中國大陸鏡像** — 自動偵測系統語言，簡體中文環境使用 hf-mirror.com 加速下載
- **多引擎 ASR** — 離線引擎（預設）、騰訊雲 ASR、Google Gemini，可自訂優先順序
- **自動 Fallback** — 設定優先順序，前一引擎失敗自動嘗試下一個
- **跨平台支援** — 支援 Windows、macOS、Linux
- **快捷鍵觸發** — 按住 Ctrl 鍵即可開始錄音，放開自動辨識並貼上
- **傾聽模式** — 連按兩下右 Ctrl 鍵開啟，連續不斷傾聽並辨識聲音，直到關閉為止
- **即時字幕模式** 🧪 — 擷取系統音訊輸出進行即時辨識，多行字幕顯示於 Overlay，關閉時自動儲存記錄檔（中途 SenseVoice 雙色即時辨識為實驗性功能）
- **會議模式** 🧪 — 同時擷取麥克風（近端）與系統音訊（遠端），雙 Overlay 即時顯示雙方辨識結果，內建回音抑制，聲紋辨識自動標記發言人，關閉時自動儲存帶時間戳、來源與說話人標示的合併文字記錄
- **系統托盤運行** — 背景運行，不干擾日常工作流程
- **記錄檔捷徑** — 托盤選單一鍵開啟記錄檔目錄
- **繁簡轉換** — 自動根據系統地區轉換中文繁簡體，串流與最終結果皆適用
- **多語言 UI** — 支援繁體中文、简体中文、English 介面
- **開機自啟** — 支援系統開機自動啟動功能
- **剪貼簿保護** — 貼上後自動恢復原有剪貼簿內容

## 支援的 ASR 引擎

### 本地雙引擎（預設）
- **串流 Zipformer**（即時）— 中英雙語串流辨識，邊說邊顯示
- **SenseVoice-Small (INT8)**（離線修正）— 高精度離線辨識，支援中英日韓語，內建 ITN 直接輸出標點
- 特色：全程離線、零隱私疑慮、首次啟動自動下載模型
- 模型儲存位置：`C:\ProgramData\VoiceTray\models\`（Windows）

### 騰訊雲 ASR
- **SentenceRecognition API** - 同步辨識
- 支援引擎：16k_zh、16k_zh_video、16k_en、16k_yue、16k_ca
- 需要：Secret ID + Secret Key

### Google Gemini
- **Multimodal generateContent API** - AI 加持語音辨識
- 支援模型：gemini-2.5-flash、gemini-2.5-flash-lite
- 自動標點符號、移除語氣詞、可指定輸出語言
- 需要：API Key

## 快速開始

### 安裝

請至[Releases頁面](https://github.com/ascetic168/VoiceTray/releases)下載安裝檔。

### 首次啟動

1. 啟動後自動開啟設定視窗
2. 點擊「下載模型」按鈕，自動下載串流、離線辨識模型
3. 下載完成後即可使用，無需設定 API 金鑰

### 使用方式

1. 按住 Ctrl 鍵開始說話（Overlay 浮動視窗即時顯示串流辨識結果）
2. 放開 Ctrl 鍵，SenseVoice 自動進行高精度修正並加上標點符號
3. 辨識結果自動貼上到當前應用程式
4. 原有剪貼簿內容自動恢復

## 技術架構

```
使用者按下 Ctrl
       ↓
  ContinuousRecorder (cpal 持續錄音流，48kHz 立體聲)
       ↓
  音訊 chunks → 降混 mono → 降採樣 16kHz
       ↓
  ┌──────────────────────────────────────────┐
  │       Silero VAD 語音活動偵測            │
  │  Pre-buffer 機制保留語音開頭上下文        │
  │  靜音片段自動過濾，不送入辨識引擎         │
  └──────────────────────────────────────────┘
       ↓ (語音 chunks only)
  ┌──────────────────────────────────────────┐
  │          串流辨識線程 (Zipformer)         │
  │  即時產生 partial text → Overlay 顯示     │
  │  Endpoint 偵測 → 片段音訊送 SenseVoice   │
  └──────────────────────────────────────────┘
       ↓ (放開 Ctrl)
  StreamingFinished → 剩餘音訊送 SenseVoice
       ↓
  ┌──────────────────────────────────────────┐
  │        SenseVoice 修正線程 (離線)         │
  │  高精度辨識 → 取代串流結果               │
  └──────────────────────────────────────────┘
       ↓
  SenseVoice 內建 ITN 輸出標點（自動加上。，？等標點）
       ↓
  convert.rs (簡繁轉換，依系統地區)
       ↓
  clipboard.rs (保存原剪貼簿 → 設定文字 → Ctrl+V → 恢復剪貼簿)
```

## 授權

閉源軟體，免費使用。由朱國棟 (Charlie Chu) 開發。
