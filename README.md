...
✅ **已發布 v0.3.3** — 加入標點符號自動還原（CT-Transformer），辨識結果更可讀。
...
- **自動標點還原** — CT-Transformer 模型自動加上。，？等標點符號，辨識結果更可讀
- **離線辨識** — 無需網路及 API 金鑰，首次啟動自動下載模型（含標點模型）
...
- **CT-Transformer**（標點還原）— 自動加上標點符號，辨識結果更可讀
- 特色：全程離線、零隱私疑慮、首次啟動自動下載模型（含標點模型）
